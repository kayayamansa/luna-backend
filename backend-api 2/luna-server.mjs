...
const formatOrb = (value) => {
  const orb = Number(value || 0);
  return `${orb.toFixed(2)}°`;
};

const formatAbsoluteDegree = (value) => {
  const normalized = normalizeLon(Number(value || 0));
...
const formatZodiacPosition = (longitude) => {
  const normalized = normalizeLon(Number(longitude || 0));
  const totalMinutes = Math.round(normalized * 60) % (360 * 60);
  const signIndex = Math.floor(totalMinutes / (30 * 60));
  const degreeMinutes = totalMinutes - signIndex * 30 * 60;
  const degrees = Math.floor(degreeMinutes / 60);
  const minutes = degreeMinutes % 60;
...
const getTimeZoneOffsetHoursAtInstant = (instantMs, timeZone) => {
  ...
  const parts = formatter.formatToParts(new Date(instantMs)).reduce((acc, part) => {
    if (part.type !== "literal") acc[part.type] = part.value;
    return acc;
  }, {});
  ...
  return (zonedUtcMs - instantMs) / 3600000;
};

const inferUtcOffsetHours = ({ date, time, coordinates }) => {
  if (!date || !time || !coordinates) return null;
  try {
    const timeZone = tzLookup(coordinates.latitude, coordinates.longitude);
    const localUtcGuess = Date.UTC(date.year, date.month - 1, date.day, time.hour, time.minute, 0);
    let instantMs = localUtcGuess;
    for (let i = 0; i < 4; i += 1) {
      const offsetHours = getTimeZoneOffsetHoursAtInstant(instantMs, timeZone);
      instantMs = localUtcGuess - offsetHours * 3600000;
    }
    return {
      timeZone,
      offsetHours: getTimeZoneOffsetHoursAtInstant(instantMs, timeZone),
    };
  } catch {
    return null;
  }
...
const PLANETS = [
  ...
  { id: swisseph.SE_MEAN_APOG, key: "lilith", name: "Lilith" },
  { id: swisseph.SE_CHIRON, key: "chiron", name: "Chiron" },
  { id: swisseph.SE_PHOLUS, key: "pholus", name: "Pholus" },
...
const isLongitudeInArc = (longitude, start, end) => {
  const lon = normalizeLon(longitude);
  const arcStart = normalizeLon(start);
  const arcEnd = normalizeLon(end);
  if (arcStart <= arcEnd) {
    return lon >= arcStart && lon < arcEnd;
  }
  return lon >= arcStart || lon < arcEnd;
};

const findHouseForLongitude = (longitude, houses) => {
  if (!houses || !Array.isArray(houses.house) || houses.house.length < 12) return null;
  const cusps = houses.house.slice(0, 12).map((value) => normalizeLon(value));
  for (let idx = 0; idx < cusps.length; idx += 1) {
    const start = cusps[idx];
    const end = cusps[(idx + 1) % cusps.length];
    if (isLongitudeInArc(longitude, start, end)) {
      return idx + 1;
    }
  }
  return null;
...
const buildCalculatedPoint = ({ key, name, longitude, houses }) => {
  const pos = toSign(longitude);
  return {
    key,
    name,
    longitude,
    ...
    speed: 0,
    houseNumber: houses ? findHouseForLongitude(longitude, houses) : null,
  };
};

const buildSectLongitude = (ascendant, isDayChart, dayValue, nightValue) =>
  isDayChart ? normalizeLon(ascendant + dayValue - nightValue) : normalizeLon(ascendant + nightValue - dayValue);
...
const computeHoroscope = async (data) => {
  ...
  const inferredOffset = !Number.isFinite(data.utcOffsetHours) ? inferUtcOffsetHours({ date, time, coordinates }) : null;
  const offsetHours = Number.isFinite(data.utcOffsetHours)
    ? data.utcOffsetHours
    : inferredOffset?.offsetHours ?? parseOffsetInput(HOROSCOPE_DEFAULT_TIMEZONE_OFFSET) ?? 0;
  ...
  if (houses && Array.isArray(houses.house)) {
    planetRows.forEach((planet) => {
      planet.houseNumber = findHouseForLongitude(planet.longitude, houses);
    });
  }

  const ascendant = houses?.ascendant;
  const mc = houses?.mc;
  const sun = planetRows.find((planet) => planet.key === "sun");
  const moon = planetRows.find((planet) => planet.key === "moon");
  const venus = planetRows.find((planet) => planet.key === "venus");
  const jupiter = planetRows.find((planet) => planet.key === "jupiter");
  const mars = planetRows.find((planet) => planet.key === "mars");
  const saturn = planetRows.find((planet) => planet.key === "saturn");

  if (Number.isFinite(ascendant) && sun && moon) {
    const sunHouse = houses ? findHouseForLongitude(sun.longitude, houses) : null;
    const isDayChart = Number.isFinite(sunHouse) ? sunHouse >= 7 && sunHouse <= 12 : true;

    const fortuneLon = buildSectLongitude(ascendant, isDayChart, moon.longitude, sun.longitude);
    planetRows.push(buildCalculatedPoint({ key: "fortune", name: "Glueckspunkt", longitude: fortuneLon, houses }));

    const loveLon = venus ? buildSectLongitude(ascendant, isDayChart, venus.longitude, sun.longitude) : null;
    if (Number.isFinite(loveLon)) {
      planetRows.push(buildCalculatedPoint({ key: "lovePoint", name: "Liebespunkt", longitude: loveLon, houses }));
    }

    if (mars && saturn) {
      const deathLon = buildSectLongitude(ascendant, isDayChart, mars.longitude, saturn.longitude);
      planetRows.push(buildCalculatedPoint({ key: "deathPoint", name: "Todespunkt", longitude: deathLon, houses }));
    }

    if (jupiter && mars) {
      const vocationLon = buildSectLongitude(ascendant, isDayChart, jupiter.longitude, mars.longitude);
      planetRows.push(buildCalculatedPoint({ key: "careerPoint", name: "Berufspunkt", longitude: vocationLon, houses }));
    }
  }
  ...
  return {
    ...
    usedTimeZone: inferredOffset?.timeZone || null,
    coordinates,
    planets: planetRows,
    aspects,
    houses,
...
const formatHoroscopeResponse = (data, report) => {
  ...
  if (report.houses && Array.isArray(report.houses.house)) {
    report.houses.house.forEach((houseLon, idx) => {
      houses.push({
        title: `Haus ${idx + 1}`,
        position: formatZodiacPosition(houseLon),
      });
    });
    houses.push({
      title: "AC",
      position: formatZodiacPosition(report.houses.ascendant),
    });
    houses.push({
      title: "MC",
      position: formatZodiacPosition(report.houses.mc),
    });
  }
  ...
  return {
    ...
    usedTimeZone: report.usedTimeZone || "",
    ...
    planets: report.planets.map((planet) => ({
      ...
      title: planet.name,
      sign: planet.sign,
      houseNumber: Number.isFinite(planet.houseNumber) ? planet.houseNumber : null,
      position: formatZodiacPosition(planet.longitude),
      absoluteDegree: formatAbsoluteDegree(planet.longitude),
    })),
...
const formatHoroscopeMail = (data, report) => {
  ...
  report.planets.forEach((p) => {
    lines.push(`${p.name}: ${formatZodiacPosition(p.longitude)} (${formatAbsoluteDegree(p.longitude)})`);
  });
  lines.push("");
  lines.push("ASPEKTE");
  if (!report.aspects.length) {
    ...
    report.aspects.slice(0, 80).forEach((a) => {
      lines.push(`${a.between}: ${a.aspect} (Orb ${formatOrb(a.orb)})`);
    });
  }
  lines.push("");
  ...
  if (report.houses && Array.isArray(report.houses.house)) {
    report.houses.house.forEach((h, idx) => {
      lines.push(`Haus ${idx + 1}: ${formatZodiacPosition(h)}`);
    });
    lines.push(`AC: ${formatZodiacPosition(report.houses.ascendant)}`);
    lines.push(`MC: ${formatZodiacPosition(report.houses.mc)}`);
  } else {
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`LUNA server laeuft auf http://0.0.0.0:${PORT}`);
});

...
const computeHoroscope = async (data) => {
  ...
  if (Number.isFinite(ascendant) && sun && moon) {
    ...
    const fortuneLon = isDayChart
      ? normalizeLon(ascendant + moon.longitude - sun.longitude)
      : normalizeLon(ascendant + sun.longitude - moon.longitude);
    planetRows.push(buildCalculatedPoint({ key: "fortune", name: "Glueckspunkt", longitude: fortuneLon, houses }));

    const loveLon =
      venus && isDayChart
        ? normalizeLon(ascendant + venus.longitude - sun.longitude)
        : venus
        ? normalizeLon(ascendant + sun.longitude - venus.longitude)
        : null;
    ...
    if (mars && saturn) {
      const deathLon = isDayChart
        ? normalizeLon(ascendant + mars.longitude - saturn.longitude)
        : normalizeLon(ascendant + saturn.longitude - mars.longitude);
      planetRows.push(buildCalculatedPoint({ key: "deathPoint", name: "Todespunkt", longitude: deathLon, houses }));
    }

    if (jupiter && mars) {
      const vocationLon = isDayChart
        ? normalizeLon(ascendant + jupiter.longitude - mars.longitude)
        : normalizeLon(ascendant + mars.longitude - jupiter.longitude);

...
const computeHoroscope = async (data) => {
  ...
  const mc = houses?.mc;
  const eighthHouse = houses?.house?.[7];
  ...
  if (Number.isFinite(ascendant) && sun && moon) {
    ...
    if (Number.isFinite(eighthHouse)) {
      const deathLon = isDayChart
        ? normalizeLon(ascendant + eighthHouse - moon.longitude)
        : normalizeLon(ascendant + moon.longitude - eighthHouse);
      planetRows.push(buildCalculatedPoint({ key: "deathPoint", name: "Todespunkt", longitude: deathLon, houses }));
    }

    if (Number.isFinite(mc)) {
      const vocationLon = normalizeLon(ascendant + mc - sun.longitude);
